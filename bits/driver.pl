...
#
# Note: The driver can use either btest.bin or the BDD checker to check
# puzzles for correctness. This version of the lab uses btest.bin, which
...
# Set to 1 to use btest.bin, 0 to use the BDD checker.
my $USE_BTEST = 1;
...
#
if ($USE_BTEST) {
    print "\n2. Compiling and running './btest.bin -g' to determine correctness score.\n";
    system("cp zap-bits.c bits.c");

    # Compile btest
    system("make btestexplicit") == 0
	or die "$0: Could not make btest.bin in $tmpdir. $diemsg\n";

    # Run btest.bin
    $status = system("./btest.bin -g > btest-zapped.out 2>&1");
    if ($status != 0) {
	die "$0: ERROR: btest.bin check failed. $diemsg\n";
    }
}
else {
...
#
# Run btest.bin or the bdd checker to compute performance score
#
if ($USE_BTEST) {
    print "\n4. Compiling and running './btest.bin -g -r 2' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    ...
    # Run btest.bin
    $status = system("./btest.bin -g -r 2 > btest-Zapped.out 2>&1");
    if ($status != 0) {
	die "$0: ERROR: Zapped btest.bin failed. $diemsg\n";
    }
}
else {
...
#
%puzzle_c_points = (); # Correctness score computed by btest.bin
%puzzle_c_errors = (); # Correctness error discovered by btest.bin
