...
static word_t* last_allocated_block = NULL; // a global variable to keep track of the address of the last allocated block
...
static word_t* search(size_t ideal_block_size) {
  //printf("search(%zu)\n",ideal_block_size);
  ...
  //if (NEXT_FIT_POLICY == 1){
    if (ORIGIN_BLOCK_ADDR != last_allocated_block){
      //printf("last_allocated_block: %p\n",last_allocated_block);
      //word_t* block = PADD(last_allocated_block,status_size(LOAD(last_allocated_block))); //start searching from the last allocated block
      word_t* block = last_allocated_block;
      //printf("block: %p\n",block);
      while (block != HEAP_FOOTER_ADDR) {
        // Get block header.
        word_t header = LOAD(block);  //load header information from block
        //printf("headerInfo: %zu\n",header);
  ...
  //}
//   //we stop if we reach the heap footer
...
//   printf("block after not finding one: %p\n",block);
//   printf("last allocated block here: %p\n",last_allocated_block);
//   //if (block != last_allocated_block){
//   do {
//     // Get block header.
//     word_t header = LOAD(block);  //load header information from block
//     //printf("header: %zu\n",header);
...
static void allocate(word_t* block, size_t ideal_block_size) {
  ...
  if (block_size > ideal_block_size){ /* We need to split the block if our block size is bigger than the ideal block size*/
...
static void coalesce(word_t* block) {
 ...
 if (prevUse == 0 && succUse != 0){
   ...
   //if the block we coalesce is the last allocated block, we need to update last_allocated_block
   //if (block == last_allocated_block){
      last_allocated_block = prevHeader; //update the last allocated block to be the preceding block that just coalesced with it
  // }
...
int mm_init() {
 ...
 last_allocated_block = ORIGIN_BLOCK_ADDR;
 //printf("init set last allocated block: %p\n", last_allocated_block);
 return 0;
}
