...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    strncpy(path, tracedir, 2*MAXLINE - 1);
    strncat(path, filename, 2*MAXLINE - strnlen(path, 2*MAXLINE) - 1);
    if ((tracefile = fopen(path, "r")) == NULL) {
        printf("Could not open %s in read_trace", path);
        unix_error("Could not open trace file in read_trace.");
    }
    if (fscanf(tracefile, "%d", &(trace->sugg_heapsize)) != 1
        || fscanf(tracefile, "%d", &(trace->num_ids)) != 1
        || fscanf(tracefile, "%d", &(trace->num_ops)) != 1
        || fscanf(tracefile, "%d", &(trace->weight)) != 1) {
      unix_error("Trace file is malformed.");
    }
